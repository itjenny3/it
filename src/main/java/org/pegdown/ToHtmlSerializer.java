...
public class ToHtmlSerializer implements Visitor {
    ...
    public ToHtmlSerializer(LinkRenderer linkRenderer,
        ...
        this.verbatimSerializers = Collections
                .<String, VerbatimSerializer> singletonMap(
                        VerbatimSerializer.DEFAULT,
                        DefaultVerbatimSerializer.INSTANCE);
    }

    public ToHtmlSerializer(final LinkRenderer linkRenderer,
            final Map<String, VerbatimSerializer> verbatimSerializers) {
        this(linkRenderer, verbatimSerializers, Collections
                .<ToHtmlSerializerPlugin> emptyList());
    }

    public ToHtmlSerializer(final LinkRenderer linkRenderer,
            final Map<String, VerbatimSerializer> verbatimSerializers,
            final List<ToHtmlSerializerPlugin> plugins) {
        this.linkRenderer = linkRenderer;
        this.verbatimSerializers = new HashMap<String, VerbatimSerializer>(
                verbatimSerializers);
        if (!this.verbatimSerializers.containsKey(VerbatimSerializer.DEFAULT)) {
            this.verbatimSerializers.put(VerbatimSerializer.DEFAULT,
                    DefaultVerbatimSerializer.INSTANCE);
        }
        this.plugins = plugins;
    }
    ...
    public void visit(RefImageNode node) {
        String text = printChildrenToString(node);
        String key = node.referenceKey != null ? printChildrenToString(node.referenceKey)
                : text;
    ...
    public void visit(RefLinkNode node) {
        ...
        if (refNode == null) { // "fake" reference link
            ...
            printLink(linkRenderer.render(node, refNode.getUrl(),
                    refNode.getTitle(), text));
    }
    ...
    public void visit(TableCellNode node) {
        ...
        TableColumnNode column = columns.get(Math.min(currentTableColumn,
                columns.size() - 1));

        printer.println().print('<').print(tag);
        column.accept(this);
        if (node.getColSpan() > 1)
            printer.print(" colspan=\"")
                    .print(Integer.toString(node.getColSpan())).print('"');
        printer.print('>');
        visitChildren(node);
    ...
    public void visit(TableNode node) {
        ...
        // ITJENNY
        printIndentedTag(node, "table class=\"table table-bordered\"");
    ...
    protected void printImageTag(SuperNode imageNode, String url) {
        ...
        if (url.contains("youtube.com") || url.contains("vimeo.com")) {
            printer.print("<iframe frameborder=\"0\" allowfullscreen src=\"")
                    .print(url).print("\"  alt=\"")
                    .printEncoded(printChildrenToString(imageNode))
                    .print("\"></iframe>");
        } else {
    ...
    protected void printWithAbbreviations(String string) {
        ...
        for (Map.Entry<String, String> entry : abbreviations.entrySet()) {
            ...
            while (true) {
                ...
                if (sx > 0 && Character.isLetterOrDigit(string.charAt(sx - 1)))
                    continue;
                if (ix < string.length()
                        && Character.isLetterOrDigit(string.charAt(ix))) {
                    continue;
                }
        ...
        if (expansions != null) {
            int ix = 0;
            for (Map.Entry<Integer, Map.Entry<String, String>> entry : expansions
                    .entrySet()) {
                int sx = entry.getKey();
