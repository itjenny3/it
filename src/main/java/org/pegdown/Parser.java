...
/**
 * Parboiled parser for the standard and extended markdown syntax. Builds an
 * Abstract Syntax Tree (AST) of {@link Node} objects.
 */
...
public class Parser extends BaseParser<Object> implements Extensions {
    ...
    public static ParseRunnerProvider DefaultParseRunnerProvider = new Parser.ParseRunnerProvider() {
    ...
    public Parser(Integer options, Long maxParsingTimeInMillis,
            ParseRunnerProvider parseRunnerProvider, PegDownPlugins plugins) {
    ...
    public Parser(Integer options, Long maxParsingTimeInMillis,
        ...
        this(options, maxParsingTimeInMillis, parseRunnerProvider,
                PegDownPlugins.NONE);
    }
    ...
    public Rule Root() {
        return NodeSequence(push(new RootNode()),
    ...
    public Rule BlockQuote() {
        ...
        return NodeSequence(
                OneOrMore(
                        ...
                        Line(inner),
                        ZeroOrMore(TestNot('>'), TestNot(BlankLine()),
                                Line(inner)),
                        ZeroOrMore(BlankLine(), inner.append(match()))),
                // trigger a recursive parsing run on the inner source we just
                // built
                // and attach the root of the inner parses AST
                push(new BlockQuoteNode(withIndicesShifted(
                        parseInternal(inner), (Integer) peek()).getChildren())));
    }
                ...
                OneOrMore(
                        ...
                        OneOrMore(FirstOf(
                                Sequence(
                                        ...
                                        line.append(repeat(
                                                ' ',
                                                4 - (currentIndex() - 1 - (Integer) peek()) % 4))),
                                Sequence(NotNewline(), ANY,
                                        line.append(matchedChar())))),
                        Newline(), text.appended(line.getString()).append('\n')
    ...
    public Rule FencedCodeBlock() {
        ...
        return NodeSequence(
                ...
                OneOrMore(TestNot(Newline(), CodeFence(markerLength)), ANY,
                        text.append(matchedChar())), Newline(),
                push(new VerbatimNode(text.appended('\n').getString(),
                        popAsString())), CodeFence(markerLength), drop());
    }
    ...
    public Rule CodeFence(Var<Integer> markerLength) {
        return Sequence(
                ...
                (markerLength.isSet() && matchLength() == markerLength.get())
                        || (markerLength.isNotSet() && markerLength
                                .set(matchLength())), Sp(),
    ...
    public Rule HorizontalRule() {
        return NodeSequence(
                NonindentSpace(),
                FirstOf(HorizontalRule('*'), HorizontalRule('-'),
                        HorizontalRule('_')), Sp(), Newline(),
                OneOrMore(BlankLine()), push(new SimpleNode(Type.HRule)));
    }
    ...
    public Rule AtxHeading() {
        return Sequence(AtxStart(), Optional(Sp()),
    ...
    public Rule SetextHeading() {
        return Sequence(
                // test for successful setext heading before actually building
                // it to reduce backtracking
    ...
    public Rule SetextHeading1() {
        return Sequence(SetextInline(), push(new HeaderNode(1, popAsNode())),
                ZeroOrMore(SetextInline(), addAsChild()), Newline(),
                NOrMore('=', 3), Newline());
    }

    public Rule SetextHeading2() {
        return Sequence(SetextInline(), push(new HeaderNode(2, popAsNode())),
                ZeroOrMore(SetextInline(), addAsChild()), Newline(),
                NOrMore('-', 3), Newline());
    }
    ...
    public Rule DefinitionList() {
        return NodeSequence(
                // test for successful definition list match before actually
                // building it to reduce backtracking
                TestNot(Spacechar()),
                Test(OneOrMore(TestNot(BlankLine()), TestNot(DefListBullet()),
                ...
                OneOrMore(
                        ...
                        ((SuperNode) peek(1)).getChildren().addAll(
                                popAsNode().getChildren()),
                        Optional(BlankLine())));
    }

    public Rule DefListTerm() {
        return NodeSequence(TestNot(Spacechar()), TestNot(DefListBullet()),
                push(new DefinitionTermNode()),
                OneOrMore(DefTermInline(), addAsChild()), Optional(':'),
                Newline());
    }
    ...
    public Rule BulletList() {
        ...
        return NodeSequence(ListItem(Bullet(), itemNodeCreator),
    ...
    public Rule OrderedList() {
        ...
        return NodeSequence(
                ListItem(Enumerator(), itemNodeCreator),
                push(new OrderedListNode(popAsNode())),
                ZeroOrMore(ListItem(Enumerator(), itemNodeCreator),
                        addAsChild()), ZeroOrMore(BlankLine()));
    }

    @Cached
    public Rule ListItem(Rule itemStart, SuperNodeCreator itemNodeCreator) {
        // for a simpler parser design we use a recursive parsing strategy for
        // list items:
        // we collect a number of markdown source blocks for an item, run
        // complete parsing cycle on these and attach
        ...
        return Sequence(
                ...
                Line(block),
                ZeroOrMore(Optional(CrossedOut(Indent(), temp)), NotItem(),
                        Line(temp),
                        block.append(temp.getString()) && temp.clearContents()),
                tight.get() ? push(tightFirstItem.setAndGet(itemNodeCreator
                        .create(parseListBlock(block))))
                        : fixFirstItem((SuperNode) peek(1))
                                && push(itemNodeCreator
                                        .create(parseListBlock(block
                                                .appended("\n\n")))),
                ZeroOrMore(
                        push(getContext().getCurrentIndex()),
                        FirstOf(Sequence(CrossedOut(BlankLine(), block),
                                tight.set(false)), tight.set(true)),
                        CrossedOut(Indent(), block),
                        FirstOf(DoubleIndentedBlocks(block),
                                IndentedBlock(block)),
                        (tight.get() ? push(parseListBlock(block))
                                : (tightFirstItem.isNotSet() || wrapFirstItemInPara(tightFirstItem
                                        .get()))
    ...
    public Rule DoubleIndentedBlocks(StringBuilderVar block) {
        ...
        return Sequence(
                ...
                block.append("    "),
                Line(block),
                ZeroOrMore(ZeroOrMore(BlankLine(), line.append(match())),
                        CrossedOut(Indent(), line), Indent(),
    ...
    public Rule IndentedBlock(StringBuilderVar block) {
        return Sequence(
                Line(block),
                ZeroOrMore(
                        FirstOf(Sequence(TestNot(BlankLine()),
                                CrossedOut(Indent(), block)), NotItem()),
                        Line(block)));
    }

    public Rule NotItem() {
        return TestNot(FirstOf(new ArrayBuilder<Rule>()
                .add(Bullet(), Enumerator(), BlankLine(), HorizontalRule())
                .addNonNulls(ext(DEFINITIONS) ? DefListBullet() : null).get()));
    }

    public Rule Enumerator() {
        return Sequence(NonindentSpace(), OneOrMore(Digit()), '.',
                OneOrMore(Spacechar()));
    }

    public Rule Bullet() {
        return Sequence(TestNot(HorizontalRule()), NonindentSpace(),
                AnyOf("+*-"), OneOrMore(Spacechar()));
    }
    ...
    Node parseListBlock(StringBuilderVar block) {
        ...
        setContext(context); // we need to save and restore the context since we
                             // might be recursing
        block.clearContents();
        return withIndicesShifted(innerRoot, (Integer) context.getValueStack()
                .pop());
    }
    ...
    public Rule HtmlBlock() {
        return NodeSequence(
                FirstOf(HtmlBlockInTags(), HtmlComment(),
                        HtmlBlockSelfClosing()), push(new HtmlBlockNode(
                        ext(SUPPRESS_HTML_BLOCKS) ? "" : match())),
                OneOrMore(BlankLine()));
    }

    public Rule HtmlBlockInTags() {
        StringVar tagName = new StringVar();
        return Sequence(Test(HtmlBlockOpen(tagName)), // get the type of tag if
                                                      // there is one
    ...
    public Rule HtmlTagBlock(StringVar tagName) {
        return Sequence(
                ...
                ZeroOrMore(FirstOf(HtmlTagBlock(tagName),
    ...
    public Rule HtmlBlockSelfClosing() {
        StringVar tagName = new StringVar();
        return Sequence('<', Spn1(), DefinedHtmlTagName(tagName), Spn1(),
                ZeroOrMore(HtmlAttribute()), Optional('/'), Spn1(), '>');
    }

    public Rule HtmlBlockOpen(StringVar tagName) {
        return Sequence('<', Spn1(), DefinedHtmlTagName(tagName), Spn1(),
                ZeroOrMore(HtmlAttribute()), '>');
    }

    @DontSkipActionsInPredicates
    public Rule HtmlBlockClose(StringVar tagName) {
        return Sequence('<', Spn1(), '/', OneOrMore(Alphanumeric()), match()
                .equals(tagName.get()), Spn1(), '>');
    }
    ...
    public Rule DefinedHtmlTagName(StringVar tagName) {
        return Sequence(
                ...
                tagName.isSet() && match().equals(tagName.get())
                        || tagName.isNotSet()
                        && tagName.set(match().toLowerCase())
                        && isHtmlTag(tagName.get()));
    }
    ...
    protected static final String[] HTML_TAGS = new String[] { "address",
            "blockquote", "center", "dd", "dir", "div", "dl", "dt", "fieldset",
            "form", "frameset", "h1", "h2", "h3", "h4", "h5", "h6", "hr",
            "isindex", "li", "menu", "noframes", "noscript", "ol", "p", "pre",
            "script", "style", "table", "tbody", "td", "tfoot", "th", "thead",
            "tr", "ul" };
    ...
    @MemoMismatches
    public Rule Inline() {
        return Sequence(checkForParsingTimeout(),
    ...
    public Rule NonLinkInline() {
        return FirstOf(new ArrayBuilder<Rule>()
                ...
                .add(Str(), Endline(), UlOrStarLine(), Space(), StrongOrEmph(),
                        Image(), Code(), InlineHtml(), Entity(), EscapedChar())
                .addNonNulls(
                        ext(QUOTES) ? new Rule[] { SingleQuoted(),
                                DoubleQuoted(), DoubleAngleQuoted() } : null)
        ...
        return Sequence(
                ...
                TestNot(FirstOf(
                        ...
                        Sequence(ZeroOrMore(NotNewline(), ANY), Newline(),
                                FirstOf(NOrMore('=', 3), NOrMore('-', 3)),
                                Newline()))),
                ext(HARDWRAPS) ? toRule(push(new SimpleNode(Type.Linebreak)))
    ...
    public Rule UlOrStarLine() {
        // This keeps the parser from getting bogged down on long strings of '*'
        // or '_',
        // or strings of '*' or '_' with space on each side:
        return NodeSequence(FirstOf(CharLine('_'), CharLine('*')),
                push(new TextNode(match())));
    }

    public Rule CharLine(char c) {
        return FirstOf(NOrMore(c, 4),
                Sequence(Spacechar(), OneOrMore(c), Test(Spacechar())));
    }
    ...
    public Rule EmphOrStrong(String chars) {
        return Sequence(
                ...
                OneOrMore(TestNot(EmphOrStrongClose(chars)), Inline(),
                        FirstOf(Sequence(
                        // if current inline ends with a closing char for a
                        // current strong node:
                                Test(isStrongCloseCharStolen(chars)),
                                // and composes a valid strong close:
                                chars.substring(0, 1),
                                // which is not followed by another closing char
                                // (e.g. in __strong _nestedemph___):
                                TestNot(chars.substring(0, 1)),
                                // degrade current inline emph to unclosed and
                                // mark current strong node for closing
                                stealBackStrongCloseChar()), addAsChild())),
                Optional(Sequence(EmphOrStrongClose(chars), setClosed())));

    }

    public Rule EmphOrStrongOpen(String chars) {
        return Sequence(TestNot(CharLine(chars.charAt(0))), chars,
                TestNot(Spacechar()), NotNewline());
    }
    ...
    public Rule EmphOrStrongClose(String chars) {
        return Sequence(
                ...
                FirstOf(Sequence(Test(ValidEmphOrStrongCloseNode.class
                        .equals(peek(0).getClass())), drop()),
                        Sequence(
                                TestNot(Spacechar()),
                                NotNewline(),
                                chars,
                                FirstOf((chars.length() == 2),
    ...
    protected boolean isLegalEmphOrStrongStartPos() {
        ...
        while (SuperNode.class.isAssignableFrom(lastClass)) {
            ...
            lastItem = supernode.getChildren().get(
                    supernode.getChildren().size() - 1);
            lastClass = lastItem.getClass();
        }

        return (TextNode.class.equals(lastClass) && ((TextNode) lastItem)
                .getText().endsWith(" "))
    ...
    /**
     * This method checks if current parent is a strong parent based on param
     * `chars`. If so, it checks if the latest inline node to be added as child
     * does not end with a closing character of the parent. When this is true, a
     * next test should check if the closing character(s) of the child should
     * become (part of) the closing character(s) of the parent.
     */
    protected boolean isStrongCloseCharStolen(String chars) {
        ...
        // checks if last `inline` to be added as child is not a
        // StrongEmphSuperNode
        ...
        if (StrongEmphSuperNode.class.equals(childClass)) {
            ...
            if (child.getChars().endsWith(chars.substring(0, 1))) {
                // The nested child ends with closing char for the parent, allow
                // stealing it back
                return true;
            }
        }
    ...
    /**
     * Steals the last close char by marking a previously closed emph/strong
     * node as unclosed.
     */
    protected boolean stealBackStrongCloseChar() {
        ...
        addAsChild();
        // signal parser to close StrongEmphSuperNode in next check for close
        // char
    ...
    /**
     * This method checks if the last parsed character or sequence is a valid
     * prefix for a closing char for an emph or strong sequence.
     */
    protected boolean isLegalEmphOrStrongClosePos() {
        Object lastItem = peek();
        if (StrongEmphSuperNode.class.equals(lastItem.getClass())) {
            List<Node> children = ((StrongEmphSuperNode) lastItem)
                    .getChildren();
            ...
            if (SimpleNode.class.equals(lastClass))
                return !((SimpleNode) lastItem).getType().equals(
    ...
    public Rule Link() {
        return NodeSequence(FirstOf(new ArrayBuilder<Rule>()
                ...
                .add(Sequence(Label(),
                        FirstOf(ExplicitLink(false), ReferenceLink(false))))
                .add(AutoLink()).get()));
    }

    public Rule NonAutoLink() {
        return NodeSequence(Sequence(Label(),
                FirstOf(ExplicitLink(false), ReferenceLink(false))));
    }

    @Cached
    public Rule ExplicitLink(boolean image) {
        return Sequence(Spn1(), '(', Sp(), LinkSource(), Spn1(),
                FirstOf(LinkTitle(), push("")), Sp(), ')',
                push(image ? new ExpImageNode(popAsString(), popAsString(),
                        popAsNode()) : new ExpLinkNode(popAsString(),
                        popAsString(), popAsNode())));
    }

    public Rule ReferenceLink(boolean image) {
        return Sequence(
                FirstOf(Sequence(Spn1(), push(match()), FirstOf(Label(), // regular
                                                                         // reference
                                                                         // link
                        Sequence("[]", push(null)) // implicit reference link
                        )), Sequence(push(null), push(null)) // implicit
                                                             // referencelink
                                                             // without trailing
                                                             // []
                ), push(image ? new RefImageNode((SuperNode) popAsNode(),
                        popAsString(), popAsNode()) : new RefLinkNode(
                        (SuperNode) popAsNode(), popAsString(), popAsNode())));
    }
        ...
        return FirstOf(
                ...
                Sequence(
                        OneOrMore(FirstOf(
                                ...
                                Sequence(TestNot(AnyOf("()>")), Nonspacechar(),
                                        url.append(matchedChar())))), push(url
                                .getString())), push(""));
    }
    ...
    public Rule LinkTitle(char delimiter) {
        return Sequence(
                delimiter,
                ZeroOrMore(TestNot(delimiter, Sp(), FirstOf(')', Newline())),
                        NotNewline(), ANY), push(match()), delimiter);
    }

    public Rule AutoLink() {
        return Sequence(ext(AUTOLINKS) ? Optional('<') : Ch('<'),
                FirstOf(AutoLinkUrl(), AutoLinkEmail()),
                ext(AUTOLINKS) ? Optional('>') : Ch('>'));
    }

    public Rule WikiLink() {
        return Sequence("[[", OneOrMore(TestNot(']'), ANY), // might have to
                                                            // restrict from ANY
                push(new WikiLinkNode(match())), "]]");
    }

    public Rule AutoLinkUrl() {
        return Sequence(Sequence(OneOrMore(Letter()), "://", AutoLinkEnd()),
    ...
    public Rule AutoLinkEmail() {
        return Sequence(
                Sequence(OneOrMore(FirstOf(Alphanumeric(), AnyOf("-+_."))),
                        '@', AutoLinkEnd()), push(new MailLinkNode(match())));
    }

    public Rule AutoLinkEnd() {
        return OneOrMore(
                TestNot(Newline()),
                ext(AUTOLINKS) ? TestNot(FirstOf(
                        '>',
                        Sequence(Optional(AnyOf(".,;:)}]\"'")),
                                FirstOf(Spacechar(), Newline()))))
                        : TestNot('>'), ANY);
    }
    ...
    public Rule Label() {
        return Sequence('[', push(new SuperNode()),
                OneOrMore(TestNot(']'), NonAutoLinkInline(), addAsChild()), ']');
    }

    public Rule Reference() {
        Var<ReferenceNode> ref = new Var<ReferenceNode>();
        return NodeSequence(NonindentSpace(), Label(),
                push(ref.setAndGet(new ReferenceNode(popAsNode()))), ':',
                Spn1(), RefSrc(ref), Sp(), Optional(RefTitle(ref)), Sp(),
                Newline(), ZeroOrMore(BlankLine()), references.add(ref.get()));
    }

    public Rule RefSrc(Var<ReferenceNode> ref) {
        return FirstOf(Sequence('<', RefSrcContent(ref), '>'),
                RefSrcContent(ref));
    }

    public Rule RefSrcContent(Var<ReferenceNode> ref) {
        return Sequence(OneOrMore(TestNot('>'), Nonspacechar()), ref.get()
                .setUrl(match()));
    }

    public Rule RefTitle(Var<ReferenceNode> ref) {
        return FirstOf(RefTitle('\'', '\'', ref), RefTitle('"', '"', ref),
                RefTitle('(', ')', ref));
    }

    public Rule RefTitle(char open, char close, Var<ReferenceNode> ref) {
        return Sequence(open,
                ZeroOrMore(TestNot(close, Sp(), Newline()), NotNewline(), ANY),
                ref.get().setTitle(match()), close);
    }
    ...
    public Rule Code() {
        return NodeSequence(
                Test('`'),
                FirstOf(Code(Ticks(1)), Code(Ticks(2)), Code(Ticks(3)),
                        Code(Ticks(4)), Code(Ticks(5))));
    }
    ...
    public Rule InlineHtml() {
        return NodeSequence(FirstOf(HtmlComment(), HtmlTag()),
                push(new InlineHtmlNode(ext(SUPPRESS_INLINE_HTML) ? ""
                        : match())));
    }
    ...
    public Rule HtmlTag() {
        return Sequence('<', Spn1(), Optional('/'), OneOrMore(Alphanumeric()),
                Spn1(), ZeroOrMore(HtmlAttribute()), Optional('/'), Spn1(), '>');
    }
    ...
    public Rule HtmlAttribute() {
        return Sequence(
                ...
                Optional(
                        ...
                        Spn1(),
                        FirstOf(Quoted(),
                                OneOrMore(TestNot('>'), Nonspacechar()))),
                Spn1());
    }
    ...
    public Rule HexEntity() {
        return Sequence(
                '#',
                IgnoreCase('x'),
                OneOrMore(FirstOf(Digit(), CharRange('a', 'f'),
                        CharRange('A', 'F'))));
    }
    ...
    public Rule NormalChar() {
        return Sequence(TestNot(SpecialChar()), TestNot(Spacechar()),
                NotNewline(), ANY);
    }

    public Rule EscapedChar() {
        return NodeSequence('\\', AnyOf("*_`&[]<>!#\\'\".+-(){}:|~"),
                push(new SpecialTextNode(match())));
    }
    ...
    public Rule Abbreviation() {
        ...
        return NodeSequence(NonindentSpace(), '*', Label(),
                push(node.setAndGet(new AbbreviationNode(popAsNode()))), Sp(),
                ':', Sp(), AbbreviationText(node), ZeroOrMore(BlankLine()),
                abbreviations.add(node.get()));
    }

    public Rule AbbreviationText(Var<AbbreviationNode> node) {
        return Sequence(
                NodeSequence(push(new SuperNode()),
                        ZeroOrMore(NotNewline(), Inline(), addAsChild())), node
                        .get().setExpansion(popAsNode()));
    }
    ...
    public Rule Table() {
        ...
        return NodeSequence(
                ...
                Optional(
                        NodeSequence(TableRow(), push(1, new TableHeaderNode())
                                && addAsChild(),
                ...
                TableDivider(node),
                Optional(
                        NodeSequence(TableRow(), push(1, new TableBodyNode())
                                && addAsChild(),
                                ZeroOrMore(TableRow(), addAsChild())),
                        addAsChild() // add the TableHeaderNode to the TableNode
                ),
                // only accept as table if we have at least one header or at
                // least one body
                Optional(TableCaption(), addAsChild()), !node.get()
                        .getChildren().isEmpty()
    ...
    public Rule TableCaption() {
        return Sequence(CaptionStart(), Optional(Sp()),
                OneOrMore(CaptionInline(), addAsChild()),
                Optional(Sp(), Optional(']'), Sp()), Newline());
    }

    public Rule CaptionStart() {
        return Sequence("[", push(new TableCaptionNode()));
    }
    ...
    public Rule TableDivider(Var<TableNode> tableNode) {
        ...
        return Sequence(Optional('|', pipeSeen.set(Boolean.TRUE)),
                OneOrMore(TableColumn(tableNode, pipeSeen)), pipeSeen.get()
                        || tableNode.get().hasTwoOrMoreDividers(), Sp(),
                Newline());
    }

    public Rule TableColumn(Var<TableNode> tableNode, Var<Boolean> pipeSeen) {
        Var<TableColumnNode> node = new Var<TableColumnNode>(
                new TableColumnNode());
        return Sequence(Sp(), Optional(':', node.get().markLeftAligned()),
                Sp(), OneOrMore('-'), Sp(),
                Optional(':', node.get().markRightAligned()), Sp(),
                Optional('|', pipeSeen.set(Boolean.TRUE)), tableNode.get()
                        .addColumn(node.get()));
    }
    ...
    public Rule TableCell() {
        return NodeSequence(
                ...
                TestNot(Sp(), Optional(':'), Sp(), OneOrMore('-'), Sp(),
                        Optional(':'), Sp(), FirstOf('|', Newline())),
                Optional(Sp(), TestNot('|'), NotNewline()),
                OneOrMore(TestNot('|'), TestNot(Sp(), Newline()), Inline(),
                        addAsChild(),
                        Optional(Sp(), Test('|'), Test(Newline()))),
                ZeroOrMore('|'), ((TableCellNode) peek()).setColSpan(Math.max(
                        1, matchLength())));
    }
    ...
    public Rule Smarts() {
        return NodeSequence(FirstOf(
                Sequence(FirstOf("...", ". . ."), push(new SimpleNode(
                        Type.Ellipsis))),
    ...
    public Rule SingleQuoted() {
        return NodeSequence(
                !Character.isLetter(getContext().getInputBuffer().charAt(
                        getContext().getCurrentIndex() - 1)), '\'',
    ...
    public Rule DoubleAngleQuoted() {
        return NodeSequence(
                ...
                Optional(
                        NodeSequence(Spacechar(),
                                push(new SimpleNode(Type.Nbsp))), addAsChild()),
                OneOrMore(FirstOf(
                        Sequence(
                                NodeSequence(OneOrMore(Spacechar()),
                                        Test(">>"), push(new SimpleNode(
                                                Type.Nbsp))), addAsChild()),
    ...
    public RootNode parseInternal(char[] source) {
        ...
        if (result.hasErrors()) {
            throw new RuntimeException(
                    "Internal error during markdown parsing:\n--- ParseErrors ---\n"
