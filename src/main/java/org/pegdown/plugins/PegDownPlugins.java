 ...
 * 
 * Construct this using @{link PegdownPlugins#builder}, and then passing in
 * either the Java plugin classes, or precompiled rules (for greater control, or
 * if using Scala rules).
 */
public class PegDownPlugins {
    ...
    public static Builder builder(PegDownPlugins like) {
        return builder().withInlinePluginRules(like.getInlinePluginRules())
                .withBlockPluginRules(like.getBlockPluginRules());
    }
    ...
    public static class Builder {
        ...
        /**
         * Add a plugin parser. This should either implement
         * {@link InlinePluginParser} or {@link BlockPluginParser}, or both. The
         * parser will be enhanced by parboiled before its rules are extracted
         * and registered here.
         * 
         * @param pluginParser
         *            the plugin parser class.
         * @param arguments
         *            the arguments to pass to the constructor of that class.
         */
        public Builder withPlugin(
                Class<? extends BaseParser<Object>> pluginParser,
                Object... arguments) {
            // First, check that the parser implements one of the parser
            // interfaces
            if (!(InlinePluginParser.class.isAssignableFrom(pluginParser) || BlockPluginParser.class
                    .isAssignableFrom(pluginParser))) {
                throw new IllegalArgumentException(
                        "Parser plugin must implement a parser plugin interface to be useful");
            }
            BaseParser<Object> parser = Parboiled.createParser(pluginParser,
                    arguments);
            if (parser instanceof InlinePluginParser) {
                withInlinePluginRules(((InlinePluginParser) parser)
                        .inlinePluginRules());
            }
            if (parser instanceof BlockPluginParser) {
                withBlockPluginRules(((BlockPluginParser) parser)
                        .blockPluginRules());
            }
            return this;
        }
        ...
        public PegDownPlugins build() {
            return new PegDownPlugins(inlinePluginRules.toArray(new Rule[0]),
                    blockPluginRules.toArray(new Rule[0]),
