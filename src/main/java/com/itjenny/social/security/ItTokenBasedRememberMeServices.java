...
public class ItTokenBasedRememberMeServices extends AbstractRememberMeServices {
	...
	public ItTokenBasedRememberMeServices() {
	}

	public ItTokenBasedRememberMeServices(String key, UserDetailsService userDetailsService) {
	...
	protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request,
		...
		ItUser userDetails = getItUserDetails(providerType, cookieTokens[0]);
	...
	protected Authentication createSuccessfulAuthentication(HttpServletRequest request, UserDetails user) {
		...
		ItUser itUser = (ItUser) user;
		auth.setDetails(itUser.getProviderType());
		return auth;
	}

	private ItUser getItUserDetails(ProviderType providerType, String firstCookieToken) {
		if (providerType == ProviderType.it) {
			ItUserDetailsService itUserDetailsService = (ItUserDetailsService) getUserDetailsService();
			return (ItUser) itUserDetailsService.loadUserByEmail(firstCookieToken);
		} else {
			return (ItUser) getUserDetailsService().loadUserByUsername(firstCookieToken);
		}
	}
	...
	public void onLoginSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (!StringUtils.hasLength(password)) {
			ItUser user = getItUserDetails(providerType, username);
		...
		setCookie(new String[] { username, Long.toString(expiryTime), signatureValue, providerType.name() },
				tokenLifetime, request, response);

		if (logger.isDebugEnabled()) {
			logger.info("Added remember-me cookie for user '" + username + "', expiry: '" + new Date(expiryTime) + "'");
		}
	}
