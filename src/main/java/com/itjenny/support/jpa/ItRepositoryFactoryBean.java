...
public class ItRepositoryFactoryBean<T extends Repository<S, ID>, S, ID extends Serializable> extends
	...
	protected static class ItCommonRepositoryFactory extends JpaRepositoryFactory {
		...
		protected <T, ID extends Serializable> JpaRepository<?, ?> getTargetRepository(RepositoryMetadata metadata,
			...
			ItCommonRepositoryImpl<?, ?> repo = new ItCommonRepositoryImpl(entityInformation, entityManager);
		...
		protected boolean isItCommonRepository(Class<?> repositoryInterface) {
			...
			return ItCommonRepository.class.isAssignableFrom(repositoryInterface);
		}
	}
}
