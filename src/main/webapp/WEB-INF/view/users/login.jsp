...
<div>
	<div class="span4">
		...
		<form id="authentication" action="/users/authenticate" method="POST">
			<input id="authenticationId" name="authenticationId" class="span3" placeholder="이메일" type="email" required value="" />
			<input id="authenticationPassword" name="authenticationPassword" class="span3" placeholder="비밀번호" type="password"
				value="" />
			<c:if test="${not empty param.login_error}">
				<label class="error" style="">E-mail or password isn't corect.</label>
			</c:if>
			<p>
				<label><input name="_spring_security_remember_me" type="checkbox" value="true" />Remember</label>
	...
	<div class="span4">
		...
		<form:form modelAttribute="user" cssClass="form-write" action="/users" method="post">
			<form:input path="email" class="inp_email" type="email" placeholder="Email" />
			<form:input path="userId" class="inp_pw" placeholder="Nickname" />
			<p>
				- Send temporary password your e-mail.
			</p>
